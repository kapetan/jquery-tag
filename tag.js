(function($) {
	...
	var Input = function(element, select, options) {
		...
		var adjustWidth = function(input, text) {
			...
			var width = input.editor('width', text); //input.textSize(text).width + margin;
		...
		var remove = this._remove = function(tag) {
			...
			if(prevInput.length && nextInput.length) {
				...
				prevInput
					...
					//.cursor(prevValue.length);

				adjustWidth(prevInput);
			} else if(nextInput.length) {
				nextInput.focus().editor('cursor', 0); //.cursor(0);
			}
		};
		var commit = function(input, cursor) {
			...
			cursor = cursor === undefined ? input.editor('cursor') : cursor; //input.cursor() : cursor;
			var text = input.val();
			...
			child.focus().editor('cursor', 0); //cursor(0);
		};
		...
		element
			.on('keydown', 'input', function(e) {
				...
				var selection = $this.editor('selection'); //.selection(); //$this.editor('selection'); //.selection();
				...
				switch(keyCode) {
					...
					case KEYS.arrowLeft:
						if(!cursor && !isSelected) {
							...
							if(prev.length) {
								prev.focus().editor('cursor', prev.val().length + 1); //.cursor(prev.val().length + 1);
								//editor.cursor(prev.val() + 1);
								e.preventDefault();
							}
						}
					...
					case KEYS.arrowRight:
						if(cursor === $this.val().length && !isSelected) {
							...
							if(next.length) {
								next.focus().editor('cursor', 0); //.cursor(0);
								//editor.cursor(0);
								e.preventDefault();
							}
						}
			...
			.on('keyup', 'input', function() {
				var $this = $(this);
				var value = $this.editor('word'); //$this.word();
			...
			.on('click', n('.fixed'), function() {
				$(this).nextAll('input').first().focus().editor('cursor', 0); //.cursor(0);
				return false;
			})
			.on('click', function(e) {
				var target = $(e.target);
	...
	Input.prototype.focus = function() {
		...
		input.focus().editor('cursor', value.length); //.cursor(value.length);
	};
	...
	Select.prototype.options = function(concealed) {
		var options = this._element.find(n('.option')); //.not('.concealed');
